...
template <typename treetype>class segtree
{
    ...
    {
        return {operand1.first+operand2.first,max(max(operand1.second,operand1.first+operand2.second),0ll)};
    }
    void merge(int pos)
    {
...
int32_t main()
{
    ...
    int n,q,m,ch;
    cin >> n >> q;
    segtree<pair<int,int>> a(n,{0,0});
    a.input();
    while(q--)
    {
        cin >> ch >> n >> m;
        if(ch==1)a.update(n,{m,m});
        else cout << a.query(n,m).second << endl;
    }
}